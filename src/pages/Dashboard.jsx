...
import { parseBusinessDate } from "../utils/dateFilterRanges";
import {
...
import "../App.css";
import {
  applyNetProfitSettingsByCurrency,
  getRoutableWalletTotals,
  normalizeKpiRouting,
} from "../utils/financialSettings";
import {
  calculateSupplierRowPaid,
  getSupplierAdjustmentDelta,
} from "../utils/supplierLedgerAccounting";


const DASHBOARD_DATE_FILTER_STORAGE_KEY = "smart-office-dashboard-date-filter";
const DASHBOARD_CUSTOM_RANGE_STORAGE_KEY = "smart-office-dashboard-custom-range";
const DASHBOARD_DATE_FILTERS = ["All", "Today", "Yesterday", "Last Week", "Last Month", "Custom"];

const readStoredDashboardDateFilter = () => {
  if (typeof window === "undefined") return "Last Month";
  try {
    const stored = window.localStorage.getItem(DASHBOARD_DATE_FILTER_STORAGE_KEY);
    return DASHBOARD_DATE_FILTERS.includes(stored) ? stored : "Last Month";
  } catch {
    return "Last Month";
  }
};

const readStoredDashboardCustomRange = () => {
  if (typeof window === "undefined") return { from: "", to: "" };
  try {
    const stored = JSON.parse(window.localStorage.getItem(DASHBOARD_CUSTOM_RANGE_STORAGE_KEY) || "null");
    return {
      from: typeof stored?.from === "string" ? stored.from : "",
      to: typeof stored?.to === "string" ? stored.to : "",
    };
  } catch {
    return { from: "", to: "" };
  }
...
const compactCurrencyTotals = (totalsByCurrency = {}, fallbackCurrency = "AFN") => {
  ...
  if (!rows.length) {
    return formatCurrencyAmount(0, fallbackCurrency).replace(/\.00(\D|$)/, "$1").trim();
  }
  return rows
    .map(([currency, amount]) => formatCurrencyAmount(amount, currency).replace(/\.00(\D|$)/, "$1").trim())
    .join("\n");
};

const explicitGregorianDateFields = [
  "gregorianDate",
  "gregorian_date",
  "dateGregorian",
];
...
const businessDateFields = [
  "date", "createdDate", "invoiceDate", "billDate", "saleDate", "paymentDate",
  "purchaseDate", "registrationDate", "transactionDate", "expenseDate", "issueDate",
  "shamsiDate",
];

const fallbackTimestampFields = ["createdAt", "updatedAt"];
...
const getDateRange = (filter, customRange) => {
  ...
  if (filter === "Last Month") {
    ...
    return { from: toDateValue(firstDay), to: toDateValue(lastDay) };
  }
  if (filter === "Custom") return customRange;
  return { from: "", to: "" };
};

const normalizeDashboardDate = (raw) => {
  const parsed = parseBusinessDate(raw);
  return parsed ? toDateValue(parsed) : "";
};

const recordDate = (record) => {
  if (!record) return "";

  for (const field of explicitGregorianDateFields) {
    const normalized = normalizeDashboardDate(record?.[field]);
    if (normalized) return normalized;
  }

  for (const field of businessDateFields) {
    const normalized = normalizeDashboardDate(record?.[field]);
    if (normalized) return normalized;
  }

  for (const field of fallbackTimestampFields) {
    const normalized = normalizeDashboardDate(record?.[field]);
    if (normalized) return normalized;
  }
...
const isInventoryPurchaseExpense = (expense) =>
  /product-purchase|product-registration|inventory|stock|godown|staff-payroll/.test(
    normalize(
      [
        expense.source,
...
function Dashboard({ t = {} }) {
  ...
  const [dateFilter, setDateFilter] = useState(readStoredDashboardDateFilter);
  const [customRange, setCustomRange] = useState(readStoredDashboardCustomRange);
  ...
  useEffect(() => {
    ...
    try {
      window.localStorage.setItem(DASHBOARD_DATE_FILTER_STORAGE_KEY, dateFilter);
    } catch {
      // Dashboard filtering must continue to work even if storage is unavailable.
    }
  }, [dateFilter]);
  ...
  useEffect(() => {
    if (typeof window === "undefined") return;
    try {
      window.localStorage.setItem(DASHBOARD_CUSTOM_RANGE_STORAGE_KEY, JSON.stringify(customRange));
    } catch {
      // Dashboard filtering must continue to work even if storage is unavailable.
    }
  }, [customRange]);
  ...
  const metrics = useMemo(() => {
    ...
    const paidRevenueByCurrency = filteredInvoices.reduce(
      (totals, invoice) =>
        addCurrencyTotal(totals, invoice.currency || baseCurrency, invoicePaid(invoice)),
      {}
    );
    ...
    const kpiRouting = normalizeKpiRouting(company);
    const routedWalletByCurrency = getRoutableWalletTotals(filteredTransactions, baseCurrency);
    const routedWalletBase = Object.entries(routedWalletByCurrency).reduce((sum, [currency, amount]) => sum + toBase(amount, currency), 0);
    const baseRevenueByCurrency = subtractCurrencyTotals(grossRevenueByCurrency, refundByCurrency);
    const revenueByCurrency = kpiRouting.totalRevenue
      ? Object.entries(routedWalletByCurrency).reduce((next, [currency, amount]) => addCurrencyTotal(next, currency, amount), baseRevenueByCurrency)
      : baseRevenueByCurrency;
    const revenue = grossRevenue - refundTotal + (kpiRouting.totalRevenue ? routedWalletBase : 0);
...
const recognizedRevenue = profitRecognizedInvoices.reduce(
  (sum, invoice) =>
    sum + toBase(invoiceTotal(invoice), invoice.currency),
  0
);

const recognizedRefundByCurrency = profitRecognizedInvoices.reduce(
  (totals, invoice) => {
    const refunds = invoice.refundHistory || invoice.refunds || [];
    return refunds.reduce(
      (nextTotals, refund) =>
        addCurrencyTotal(
          nextTotals,
          refund.currency || invoice.currency || baseCurrency,
          refund.amount
        ),
      totals
    );
...
const recognizedRefundTotal = profitRecognizedInvoices.reduce(
  (sum, invoice) => {
    const refunds = invoice.refundHistory || invoice.refunds || [];
    return (
      sum +
...
const recognizedGrossCostByCurrency = profitRecognizedInvoices.reduce(
  (totals, invoice) =>
    addCurrencyTotal(
      totals,
      invoice.currency || baseCurrency,
...
const recognizedGrossCost = profitRecognizedInvoices.reduce(
  (sum, invoice) =>
    sum +
    toBase(
      invoiceCost(invoice, filteredProducts, { baseCurrency, exchangeRates }),
...
const recognizedRefundCostByCurrency = profitRecognizedInvoices.reduce(
  (totals, invoice) =>
    addCurrencyTotal(
      totals,
      invoice.currency || baseCurrency,
...
const recognizedRefundCost = profitRecognizedInvoices.reduce(
  (sum, invoice) =>
    sum +
    toBase(
      invoiceRefundCost(invoice, filteredProducts, { baseCurrency, exchangeRates }),
...
const recognizedNetRevenueByCurrency = subtractCurrencyTotals(
  recognizedRevenueByCurrency,
  recognizedRefundByCurrency
);

const recognizedSoldGoodsCostByCurrency = subtractCurrencyTotals(
  recognizedGrossCostByCurrency,
  recognizedRefundCostByCurrency
);

const recognizedSoldGoodsCost = Math.max(
  0,
...
const pureProfitByCurrency = kpiRouting.pureProfit
  ? Object.entries(routedWalletByCurrency).reduce(
      (next, [currency, amount]) => addCurrencyTotal(next, currency, amount),
...
const baseNetProfitByCurrency = subtractCurrencyTotals(
  subtractCurrencyTotals(basePureProfitByCurrency, operatingExpenseByCurrency),
  staffPaid ? { [baseCurrency]: staffPaid } : {}
);
const routedNetProfitByCurrency = kpiRouting.netProfit
  ? Object.entries(routedWalletByCurrency).reduce((next, [currency, amount]) => addCurrencyTotal(next, currency, amount), baseNetProfitByCurrency)
  : baseNetProfitByCurrency;
const netProfitRuleResult = applyNetProfitSettingsByCurrency(routedNetProfitByCurrency, company);
const netProfitByCurrency = netProfitRuleResult.totals;
    ...
    // Keep Supplier / Katah dashboard balances in sync with the Suppliers page.
    // A supplier's live balance is not only supplier.balance; unpaid Godown
    // purchases and manual supplier adjustments also change the account.
    const supplierBalanceStats = new Map();
    filteredSuppliers
      .filter((supplier) => !supplier?.accountType || supplier.accountType === "supplier")
      .forEach((supplier) => {
        const key = String(supplier.id || supplier.supplierId || "");
        if (!key) return;
        supplierBalanceStats.set(key, {
        ...
        rows.reduce(
          (sum, row) =>
            ...
            (parseNumber(row?.total) ||
              parseNumber(row?.quantity) * parseNumber(row?.purchase)),
          0
        );
      const billPaid = Math.min(
        Math.max(0, parseNumber(entry?.paid)),
        Math.max(0, billTotal)
      );

      rows.forEach((row) => {
        const supplierId = String(row?.supplierId || entry?.supplierId || "");
        const stat = supplierBalanceStats.get(supplierId);
        if (!stat) return;
        const total =
          parseNumber(row?.total) ||
          parseNumber(row?.quantity) * parseNumber(row?.purchase);
        const paid = calculateSupplierRowPaid({
          rowTotal: total,
          rowPaid: row?.paid,
          billPaid,
          billTotal,
        });
        stat.balance += Math.max(0, total - paid);
      });

      (Array.isArray(entry?.adjustments) ? entry.adjustments : []).forEach(
        (adjustment) => {
          const supplierId = String(adjustment?.supplierId || "");
          const stat = supplierBalanceStats.get(supplierId);
          if (!stat) return;
          stat.balance += getSupplierAdjustmentDelta(adjustment);
        }
      );
    });

    const supplierPayablesByCurrency = {};
    const supplierReceivablesByCurrency = {};
    [...supplierBalanceStats.values()].forEach((stat) => {
      const currency = stat.currency || baseCurrency;
      addCurrencyTotal(supplierPayablesByCurrency, currency, Math.max(0, parseNumber(stat.balance)));
      addCurrencyTotal(supplierReceivablesByCurrency, currency, Math.max(0, -parseNumber(stat.balance)));
    });
    const supplierNetByCurrency = subtractCurrencyTotals(supplierPayablesByCurrency, supplierReceivablesByCurrency);
    const supplierPayables = Object.values(supplierPayablesByCurrency).reduce((sum, value) => sum + parseNumber(value), 0);
    const supplierReceivables = Object.values(supplierReceivablesByCurrency).reduce((sum, value) => sum + parseNumber(value), 0);
    const supplierNetBalance = supplierPayables - supplierReceivables;
    ...
    const effectiveCashWalletByCurrency = kpiRouting.currentCashWallet
      ? cashWalletByCurrency
      : subtractCurrencyTotals(cashWalletByCurrency, routedWalletByCurrency);
    const cashWalletEntries = Object.entries(effectiveCashWalletByCurrency)
      .filter(([, amount]) => parseNumber(amount) !== 0);
  ...
  supplierReceivablesByCurrency,
  supplierNetByCurrency,

  totalCustomers: customers.length,
  totalInvoices: filteredInvoices.length,
  totalStaff: filteredStaff.length,
};
  }, [customers, filteredExpenses, filteredInvoices, filteredProducts, filteredStaff, filteredSuppliers, filteredTransactions, godownEntries, products, baseCurrency, displayCurrency, exchangeRates, toBase, company]);
...
const statCards = [
  ...
  // Supplier / Katah overview
  {
    ...
    value: compactCurrencyTotals(metrics.supplierReceivablesByCurrency, baseCurrency),
    icon: TrendingUp,
    tone: "green",
  ...
  },
  {
    ...
    value: compactCurrencyTotals(metrics.supplierPayablesByCurrency, baseCurrency),
    subValue: t.amountOwedToSuppliers || "Amount owed to suppliers",
    icon: Banknote,
    tone: "orange",
  ...
  },
  {
    ...
    value: compactCurrencyTotals(metrics.supplierNetByCurrency, baseCurrency),
    subValue: t.netBalance || "Net Balance",
    icon: DollarSign,
    tone: "orange",
