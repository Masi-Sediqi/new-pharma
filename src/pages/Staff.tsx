...
const ChevronDown = (p:IconProps)=><SvgIcon {...p}><path d="m6 9 6 6 6-6"/></SvgIcon>
...
const multiMoney=(totals:Record<string,number>)=>{
 ...
 entries.sort(([a],[b])=>(order.indexOf(a)<0?999:order.indexOf(a))-(order.indexOf(b)<0?999:order.indexOf(b))||a.localeCompare(b))
...
export default function Staff({language,onOpenStaff,globalSearch=''}:{language:Language;onOpenStaff:(id:string)=>void;globalSearch?:string}){
 ...
 const salaryByCurrency:Record<string,number>={}, paidByCurrency:Record<string,number>={}, payableByCurrency:Record<string,number>={}
 rows.forEach((s:any)=>{addMoney(salaryByCurrency,s.currency||'AFN',n(s.salary));addMoney(payableByCurrency,s.currency||'AFN',payable(s));(Array.isArray(s.payrollHistory)?s.payrollHistory:[]).forEach((p:any)=>addMoney(paidByCurrency,p.currency||s.currency||'AFN',n(p.paidAmount)))})
 ...
 return <div className="w-full pb-8">
  ...
  <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4"><Card icon={<Users/>} label={t.total} value={String(rows.length)} tone="blue"/><Card icon={<CircleDollarSign/>} label={t.salary} value={multiMoney(salaryByCurrency)} tone="orange"/><Card icon={<WalletCards/>} label={t.paid} value={multiMoney(paidByCurrency)} tone="green"/><Card icon={<UserPlus/>} label={t.payable} value={multiMoney(payableByCurrency)} tone="orange"/></div>
...
function SmoothSelect({value,onChange,options}:{value:string;onChange:(v:string)=>void;options:{value:string;label:string}[]}){
 ...
 return <div className="relative">
  <button type="button" onClick={()=>setOpen(v=>!v)} className="form-control flex items-center justify-between gap-2 text-start"><span className="truncate">{selected?.label}</span><span className="grid h-6 w-6 shrink-0 place-items-center rounded-md bg-slate-50 text-slate-500 dark:bg-white/10 dark:text-slate-200"><ChevronDown size={15} className={`transition-transform ${open?'rotate-180':''}`}/></span></button>
  {open&&<><button type="button" className="fixed inset-0 z-[119]" onClick={()=>setOpen(false)}/><div className="absolute inset-x-0 top-[calc(100%+5px)] z-[120] max-h-64 overflow-y-auto rounded-xl border border-slate-200 bg-white p-1.5 shadow-xl dark:border-[#30456f] dark:bg-[#0d1628]">{options.map(o=><button key={o.value} type="button" onClick={()=>{onChange(o.value);setOpen(false)}} className={`flex w-full items-center gap-2 rounded-lg px-3 py-2 text-start text-sm transition ${o.value===value?'bg-amber-500 font-semibold text-slate-950':'hover:bg-slate-100 dark:hover:bg-white/10'}`}><span className="w-4">{o.value===value?'✓':''}</span>{o.label}</button>)}</div></>}
 </div>
}
...
function StaffModal({t,initial,onClose,onSave}:{t:any;initial:StaffRecord|null;onClose:()=>void;onSave:(s:StaffRecord)=>void}){const blank:StaffRecord={id:'',name:'',phone:'',email:'',role:'',department:'',employmentType:'Full-time',joiningDate:new Date().toISOString().slice(0,10),salaryType:'fixed',salary:0,currency:'AFN',status:'Active',notes:'',payrollHistory:[],createdAt:'',updatedAt:''};const [f,setF]=useState<StaffRecord>(initial?{...blank,...initial}:blank);const submit=(e:FormEvent)=>{e.preventDefault();if(!f.name.trim()||!f.role.trim())return;onSave(f)};useEffect(()=>{document.body.style.overflow='hidden';return()=>{document.body.style.overflow=''}},[]);return <div className="modal-backdrop" onMouseDown={e=>{if(e.currentTarget===e.target)onClose()}}><form onSubmit={submit} className="modal-card max-w-xl overflow-visible"><div className="mb-5 flex items-center justify-between"><h2 className="text-lg font-extrabold">{initial?t.edit:t.formTitle}</h2><button type="button" onClick={onClose}><X size={18}/></button></div><div className="grid gap-3 sm:grid-cols-2"><Field l={t.name}><input className="form-control" value={f.name} onChange={e=>setF({...f,name:e.target.value})}/></Field><Field l={t.role}><input className="form-control" value={f.role} onChange={e=>setF({...f,role:e.target.value})}/></Field><Field l={t.phone}><input className="form-control" value={f.phone} onChange={e=>setF({...f,phone:e.target.value})}/></Field><Field l={t.email}><input className="form-control" value={f.email} onChange={e=>setF({...f,email:e.target.value})}/></Field><Field l={t.dept}><input className="form-control" value={f.department} onChange={e=>setF({...f,department:e.target.value})}/></Field><Field l={t.employment}><SmoothSelect value={f.employmentType} onChange={value=>setF({...f,employmentType:value})} options={[{value:'Full-time',label:'Full-time'},{value:'Part-time',label:'Part-time'},{value:'Contract',label:'Contract'}]}/></Field><Field l={t.joinDate}><input type="date" className="form-control" value={f.joiningDate} onChange={e=>setF({...f,joiningDate:e.target.value})}/></Field><Field l={t.baseSalary}><input type="number" className="form-control" value={f.salary} onChange={e=>setF({...f,salary:n(e.target.value)})}/></Field><Field l={t.currency}><SmoothSelect value={f.currency} onChange={value=>setF({...f,currency:value})} options={currencies.map(([code,symbol,name])=>({value:code,label:`${symbol} ${code} - ${name}`}))}/></Field><Field l={t.status}><SmoothSelect value={f.status} onChange={value=>setF({...f,status:value})} options={[{value:'Active',label:'Active'},{value:'Inactive',label:'Inactive'}]}/></Field></div><Field l={t.notes}><textarea className="form-control mt-3 min-h-24" value={f.notes} onChange={e=>setF({...f,notes:e.target.value})}/></Field><div className="mt-5 flex gap-2"><button className="app-btn-primary flex-1">{t.save}</button><button type="button" onClick={onClose} className="app-btn-secondary">{t.cancel}</button></div></form></div>}
